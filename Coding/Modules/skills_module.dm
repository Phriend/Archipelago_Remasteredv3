	...
	proc
		StatSkills()
			...
			for ( skill = 1, skill <= MAX_SKILLS, skill++  )
				statpanel("Skills",GetSkillName(skill),"[color][GetSkill(skill)] ([getSkillXPLeft(skill)] left)")
		...
		GetSkill(skill)
			...
			if ( equipped )
				...
				if ( skill == SKILL_COMBAT )
					...
					if ( istype(equipped,/obj/item/tool/Dagger) || istype(equipped,/obj/item/tool/Sword) || istype(equipped,/obj/item/tool/Blade) )
		...
		getSkillXPLeft(skill)
			...
			if( level == 10)
				return SKILL_XP_LEVEL_11-XP
			if( level == 11)
				return SKILL_XP_LEVEL_12-XP
			if( level == 12)
				return SKILL_XP_LEVEL_13-XP
			if( level == 13)
				return SKILL_XP_LEVEL_14-XP
			if( level == 14)
				return SKILL_XP_LEVEL_15-XP
			if( level == 15)
				return SKILL_XP_LEVEL_16-XP
			if( level == 16)
				return SKILL_XP_LEVEL_17-XP
			if( level == 17)
				return SKILL_XP_LEVEL_18-XP
			if( level == 18)
				return SKILL_XP_LEVEL_19-XP
			if( level == 19)
				return SKILL_XP_LEVEL_20-XP
			if( level == 20)
				return "NONE"
		getSkillLevel(XP)
...
mob/player/proc/AssignSkills()
	...
	setBusy(1)
	choice = input(src,"Select your First skill.","Major skill") in skillList
	...
	choice = input(src,"Select your Second skill.","Minor skill") in skillList
	setBusy(0)
