...
mob/player
	...
	proc/Invited(mob/player/clanmate,datum/tribe/newTribe)
		...
		if ( responce == "Yes" )
			...
			Tribe = new(newTribe,src)
			var datum/tribeMember/TM = newTribe.FindMember(src)
			TM.rank = TRIBE_RANK_MEMBER
	...
	verb
		...
		tSay(message as text)
			...
			Tribe.Message("<b><font color = red>[rankName] [usr] says :</b><font color = gray> [html_encode(message)]")
	...
	tribe
		...
		proc
			...
			Message(message)
				for ( var/datum/tribeMember/TM in memberList )
					if ( TM.Mob )
						TM.Mob << "<b><font color = green>\[ Clan \] </b><font color = gray>[message]"
