...
obj/building/Furnace
	...
	DblClick()
		if(!isLit)
			switch (woodLeft)
				if ( 4 to MAX_FURNACE_WOOD )	gameMessage(usr, "The furnace is overflowing with wood.",MESSAGE_FIRE)
				if ( 3 )		gameMessage(usr, "The furnace is pretty full.",MESSAGE_FIRE)
				if ( 2 )		gameMessage(usr,"The furnace has a bit of fuel.",MESSAGE_FIRE)
				if ( 1 )		gameMessage(usr, "There is almost no fuel in the furnace.",MESSAGE_FIRE)
				if ( 0 )		gameMessage(usr, "There is no fuel in the furnace.",MESSAGE_FIRE)
		else
			switch (woodLeft)
	...
	Fire
		...
		proc
			SetLumin()
				var newLum
				switch (woodLeft)
					if (3 to MAX_CAMPFIRE_WOOD)	newLum = 6
					if (2)			newLum = 5
					if (1)			newLum = 4
